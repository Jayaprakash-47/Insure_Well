...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // Connect SSE when user is already logged in (e.g. page refresh)
    if (this.auth.isLoggedIn()) {
      this.notifService.connect();
    }
    // Also connect on future logins
    this.auth.isLoggedIn$.subscribe((loggedIn) => {
      if (loggedIn) this.notifService.connect();
      else this.notifService.disconnect();
    });
  }
  ...
  getNavLinks(): { label: string; route: string; icon: string }[] {
      ...
      return [
        {
          ...
          icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z" /><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z" /></svg>',
        },
        {
          label: 'Customers',
        ...
        {
          label: 'Underwriters',
          route: '/admin/underwriters',
          icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m3.75 9v6m3-3H9m1.5-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>',
        },
        {
          label: 'Assign UW',
          route: '/admin/assign-underwriter',
          icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.671zM12 2.25V4.5m5.834.166l-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243l-1.59-1.59" /></svg>',
        },
        {
          label: 'Officers',
          route: '/admin/claims-officers',
        ...
        {
          label: 'Assign CO',
          route: '/admin/assign-claims-officer',
    ...
    if (this.auth.isClaimsOfficer()) {
      return [
        {
          label: 'Dashboard',
          route: '/claims-officer/dashboard',
          icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z" /><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z" /></svg>',
        },
        {
          label: 'My Claims',
          route: '/claims-officer/my-claims',
  ...
  getRoleBadge(): string {
    ...
    if (this.auth.isUnderwriter()) return 'Underwriter';
    return 'Customer';
  }
  ...
  getRoleBadgeClass(): string {
    ...
    if (this.auth.isUnderwriter()) return 'role-underwriter';
