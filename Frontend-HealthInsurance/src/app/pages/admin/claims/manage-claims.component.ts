...
export class ManageClaimsComponent implements OnInit {
    ...
    loadClaims(): void {
        this.api.getAllClaims().subscribe({ next: (d) => { this.claims = d; this.loading = false; this.cdr.detectChanges(); }, error: () => { this.loading = false; this.cdr.detectChanges(); } });
    }

    openAction(claim: ClaimResponse, action: string): void {
        this.selectedClaim = claim; this.action = action;
    ...
    submit(): void {
        ...
        const req = this.action === 'APPROVED'
        ...
        this.api.updateClaimStatus(this.selectedClaim.claimId, req).subscribe({
            next: () => { this.toast.success(`Claim ${this.action.toLowerCase()}`); this.showModal = false; this.processing = false; this.loadClaims(); this.cdr.detectChanges(); },
            error: (err) => { this.processing = false; this.toast.error(err.error?.message || 'Failed'); this.cdr.detectChanges(); }
        });
    }
