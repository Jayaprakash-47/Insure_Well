...
export class ManageUnderwritersComponent implements OnInit {
    ...
    validateField(field: string): void {
        switch (field) {
            ...
            case 'password':
                ...
                break;
            case 'licenseNumber':
                if (!this.form.licenseNumber?.trim()) { this.errors.licenseNumber = 'License number cannot be empty'; } else { delete this.errors.licenseNumber; }
                break;
            case 'commissionPercentage':
                if (this.form.commissionPercentage == null || this.form.commissionPercentage < 0 || this.form.commissionPercentage > 100) {
                    this.errors.commissionPercentage = 'Commission must be between 0 and 100';
                } else { delete this.errors.commissionPercentage; }
                break;
        }
    }
    ...
    createUnderwriter() {
        this.errors = {};
        this.touched = { firstName: true, lastName: true, email: true, password: true, licenseNumber: true, commissionPercentage: true };
