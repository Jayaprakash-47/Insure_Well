...
export class AssignClaimsOfficerComponent implements OnInit {
    ...
    ngOnInit() {
        this.api.getSubmittedClaims().subscribe({
            next: (d) => { this.claims = d; this.loadingClaims = false; },
            error: () => { this.loadingClaims = false; }
        });
        this.loadingOfficers = true;
        this.api.getAllClaimsOfficers().subscribe({
            next: (d) => { this.officers = d.filter((o: any) => o.isActive); this.loadingOfficers = false; },
            error: () => { this.loadingOfficers = false; }
        });
    }
    ...
    assignOfficer() {
        ...
        this.api.assignClaimsOfficer(this.selectedClaim.claimId, { claimsOfficerId: this.selectedOfficerId }).subscribe({
            next: () => {
                this.assigning = false; this.success = true;
                this.claims = this.claims.filter(c => c.claimId !== this.selectedClaim.claimId);
                setTimeout(() => { this.selectedClaim = null; this.selectedOfficerId = null; this.success = false; }, 2000);
            },
            error: (err) => {
