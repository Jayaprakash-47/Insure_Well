...
export class CalculatorComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.api.getAllPlans().subscribe({ next: (d) => { this.plans = d.filter(p => p.isActive); this.loading = false; if (this.plans.length) this.form.planId = this.plans[0].planId; this.cdr.detectChanges(); } });
        this.api.getMyQuotes().subscribe({ next: (d) => { this.quotes = d; this.cdr.detectChanges(); } });
    }

    calculate(): void {
        if (!this.form.planId) { this.toast.error('Select a plan'); return; }
        this.calculating = true; this.cdr.detectChanges();
        this.api.calculatePremium(this.form).subscribe({
            next: (res) => { this.result = res; this.calculating = false; this.quotes.unshift(res); this.cdr.detectChanges(); },
            error: (err) => { this.calculating = false; this.toast.error(err.error?.message || 'Calculation failed'); this.cdr.detectChanges(); }
        });
    }
